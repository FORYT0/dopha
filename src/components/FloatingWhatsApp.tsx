...
export default function FloatingWhatsApp() {
  ...
  // Live payment statuses keyed by checkoutRequestId (updated by Daraja callback → Firestore)
  const [paymentStatuses, setPaymentStatuses] = useState<
    Record<string, { status: 'pending' | 'success' | 'failed'; mpesaRef?: string | null }>
  >({});
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'chats', sessionId), (snap: DocumentSnapshot) => {
      ...
      // Extract payment statuses map (written by callback, avoids array mutation)
      const ps = (snap.data() as Record<string, unknown>).paymentStatuses as
        Record<string, { status: 'pending' | 'success' | 'failed'; mpesaRef?: string | null }> | undefined;
      if (ps) setPaymentStatuses(ps);
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2" style={{ background: WA_BG }}>
              ...
              {messages.map(msg => {
                ...
                if (msg.type === 'payment') {
                  // Resolve live status: callback writes to paymentStatuses map on session doc
                  const live    = msg.checkoutRequestId ? paymentStatuses[msg.checkoutRequestId] : undefined;
                  const status  = (live?.status ?? msg.paymentStatus ?? 'pending') as 'pending' | 'success' | 'failed';
                  const mRef    = live?.mpesaRef ?? msg.mpesaRef ?? null;
                  return (
                    <div key={msg.id} className="flex justify-start">
                      <div className="max-w-[88%]">
                        <PaymentCard status={status} amount={msg.amount} mpesaRef={mRef} />
                ...
                if (msg.type === 'quote' && msg.order) {
                  // Only show the Pay button if no payment exists yet for this orderRef
                  const alreadyPaid = messages.some(
                    m => m.type === 'payment' && m.orderRef === msg.orderRef
                  );
                  return (
                    <div key={msg.id} className="flex justify-start">
                      <div className="max-w-[92%]">
                        <QuoteCard items={msg.order} orderRef={msg.orderRef} total={msg.total} />
                        {!alreadyPaid && msg.total != null && msg.total > 0 && (
                          <PaySection
                            total={msg.total}
